---
title: Using Turborepo with Buildkite
description: How to use Buildkite with Turborepo to optimize your CI workflow
---

import { Tabs, Tab } from "../../../../components/Tabs";

# Using Turborepo with Buildkite

The following example shows how to use Turborepo with [Buildkite](https://buildkite.com/).

For a given root `package.json`:

```json
{
  "name": "my-turborepo",
  "scripts": {
    "build": "turbo run build",
    "test": "turbo run test"
  },
  "devDependencies": {
    "turbo": "1.2.5"
  }
}
```

And a `turbo.json`:

```json
{
  "$schema": "https://turbo.build/schema.json",
  "pipeline": {
    "build": {
      "outputs": [".next/**", "!.next/cache/**"],
      "dependsOn": ["^build"]
    },
    "test": {
      "dependsOn": ["^build"]
    },
  }
}
```

Create a file called `.buildkite/pipeline.yml` in your repository with the following contents:

<Tabs items={['npm', 'yarn', 'pnpm']} storageKey="selected-pkg-manager">
  <Tab>
    ```yaml
    name: "CI"
    description: "Builds and test the repository"
    steps:
      - label: "Install dependencies"
        key: install-dependencies
        command: npm run install

      - group: "build-and-test"
        depends_on: install-dependencies
        steps:
          - label: "Build the application"
            command: npm run build
          - label: "Test the application"
            command: npm run test
    ```
  </Tab>
  <Tab>
    ```yaml
    name: "CI"
    description: "Builds and test the repository"
    steps:
      - label: "Install dependencies"
        key: install-dependencies
        command: yarn install

      - group: "build-and-test"
        depends_on: install-dependencies
        steps:
          - label: "Build the application"
            command: yarn build
          - label: "Test the application"
            command: yarn test
    ```
  </Tab>
  <Tab>
    ```yaml
    name: "CI"
    description: "Builds and test the repository"
    steps:
      - label: "Install dependencies"
        key: install-dependencies
        command: yarn install

      - label: "Install `pnpm`"
        key: install-pnpm
        command: npm i -g pnpm

      - group: "build-and-test"
        depends_on: install-pnpm
        steps:
          - label: "Build the application"
            command: pnpm build
          - label: "Test the application"
            command: pnpm test
    ```
  </Tab>
</Tabs>

## Remote Caching

To use Remote Caching with Buildkite, add the following environment variables to your Buildkite project
to make them available to your `turbo` commands.

- `TURBO_TOKEN` - The Bearer token to access the Remote Cache
- `TURBO_TEAM` - The account to which the monorepo belongs

To use Vercel Remote Caching, you can get the value of these variables in a few steps:

1. Create a Scoped Access Token to your account in the [Vercel Dashboard](https://vercel.com/account/tokens)

![Vercel Access Tokens](/images/docs/vercel-create-token.png)

Copy the value to a safe place. You'll need it in a moment.

2. Go to your Buildkite project settings and click on the **Environment Variables** tab. Create a new secret called `TURBO_TOKEN` and enter the value of your Scoped Access Token.

![Buildkite Environment Variables](/images/docs/Buildkite-environment-variables.png)
![Buildkite Create Environment Variables](/images/docs/Buildkite-create-environment-variables.png)

3. Make a second secret called `TURBO_TEAM` and enter the value of your team's Vercel URL _without_ the `vercel.com/`. Your Team URL can be found inside your team's general project settings from the dashboard.

   If you're using a Hobby Plan, you can use your username. Your username can be found in your [Vercel Personal Account Settings](https://vercel.com/account)

![Vercel Account Slug](/images/docs/vercel-slug.png)

4. Buildkite automatically loads environment variables stored in project settings into the CI environment. No modifications are necessary for the CI file.
